...
#
#    Cmdline:
#        sokol-shdc -i instancingcompute.glsl -f sokol_nim -o instancingcompute.nim -l glsl430:metal_macos:hlsl5:glsl310es
#
#    Overview:
#    =========
...
#            Nim struct: Particle
#            Bind slot: viewVsSsbo => 0
#            Readonly: true
#        Storage buffer 'cs_ssbo':
#            Nim struct: Particle
#            Bind slot: viewCsSsbo => 0
#            Readonly: false
#
...
#
#
const csInitSourceGlsl310es: array[857, uint8] = [
...
#
#
const csUpdateSourceGlsl310es: array[916, uint8] = [
    0x23'u8,0x76,0x65,0x72,0x73,0x69,0x6f,0x6e,0x20,0x33,0x31,0x30,0x20,0x65,0x73,0x0a,
    0x6c,0x61,0x79,0x6f,0x75,0x74,0x28,0x6c,0x6f,0x63,0x61,0x6c,0x5f,0x73,0x69,0x7a,
    0x65,0x5f,0x78,0x20,0x3d,0x20,0x36,0x34,0x2c,0x20,0x6c,0x6f,0x63,0x61,0x6c,0x5f,
    0x73,0x69,0x7a,0x65,0x5f,0x79,0x20,0x3d,0x20,0x31,0x2c,0x20,0x6c,0x6f,0x63,0x61,
...
#
#
const vsSourceGlsl310es: array[453, uint8] = [
    0x23'u8,0x76,0x65,0x72,0x73,0x69,0x6f,0x6e,0x20,0x33,0x31,0x30,0x20,0x65,0x73,0x0a,
...
#
#
const fsSourceGlsl310es: array[196, uint8] = [
    0x23'u8,0x76,0x65,0x72,0x73,0x69,0x6f,0x6e,0x20,0x33,0x31,0x30,0x20,0x65,0x73,0x0a,
...
#    }
#
const vsSourceHlsl5: array[1011, uint8] = [
    0x73'u8,0x74,0x72,0x75,0x63,0x74,0x20,0x70,0x61,0x72,0x74,0x69,0x63,0x6c,0x65,0x0a,
    0x7b,0x0a,0x20,0x20,0x20,0x20,0x66,0x6c,0x6f,0x61,0x74,0x34,0x20,0x70,0x6f,0x73,
    0x3b,0x0a,0x20,0x20,0x20,0x20,0x66,0x6c,0x6f,0x61,0x74,0x34,0x20,0x76,0x65,0x6c,
...
proc displayShaderDesc*(backend: sg.Backend): sg.ShaderDesc =
    result = sg.ShaderDesc(label: "display_shader")
    case backend:
        of backendGlcore:
            ...
            result.views[0].storageBuffer.stage = shaderStageVertex
            result.views[0].storageBuffer.readonly = true
            result.views[0].storageBuffer.glslBindingN = 0
        of backendGles3:
            result.vertexFunc.source = cast[cstring](addr(vsSourceGlsl310es))
            result.vertexFunc.entry = "main"
            result.fragmentFunc.source = cast[cstring](addr(fsSourceGlsl310es))
            result.fragmentFunc.entry = "main"
            result.attrs[0].baseType = shaderAttrBaseTypeFloat
            result.attrs[0].glslName = "pos"
            result.attrs[1].baseType = shaderAttrBaseTypeFloat
        ...
        of backendD3d11:
            ...
            result.views[0].storageBuffer.stage = shaderStageVertex
            result.views[0].storageBuffer.readonly = true
            result.views[0].storageBuffer.hlslRegisterTN = 0
        of backendMetalMacos:
            ...
            result.views[0].storageBuffer.stage = shaderStageVertex
            result.views[0].storageBuffer.readonly = true
            result.views[0].storageBuffer.mslBufferN = 8
        else: discard
proc initShaderDesc*(backend: sg.Backend): sg.ShaderDesc =
    result = sg.ShaderDesc(label: "init_shader")
    case backend:
        of backendGlcore:
            result.computeFunc.source = cast[cstring](addr(csInitSourceGlsl430))
            result.computeFunc.entry = "main"
            result.views[0].storageBuffer.stage = shaderStageCompute
            result.views[0].storageBuffer.readonly = false
            result.views[0].storageBuffer.glslBindingN = 0
        of backendGles3:
            result.computeFunc.source = cast[cstring](addr(csInitSourceGlsl310es))
            result.computeFunc.entry = "main"
            result.views[0].storageBuffer.stage = shaderStageCompute
            result.views[0].storageBuffer.readonly = false
            result.views[0].storageBuffer.glslBindingN = 0
        of backendD3d11:
            ...
            result.views[0].storageBuffer.stage = shaderStageCompute
            result.views[0].storageBuffer.readonly = false
            result.views[0].storageBuffer.hlslRegisterUN = 0
        of backendMetalMacos:
...
proc updateShaderDesc*(backend: sg.Backend): sg.ShaderDesc =
    result = sg.ShaderDesc(label: "update_shader")
    case backend:
        of backendGlcore:
            ...
            result.uniformBlocks[0].glslUniforms[0].type = uniformTypeFloat
            result.uniformBlocks[0].glslUniforms[0].arrayCount = 0
            result.uniformBlocks[0].glslUniforms[0].glslName = "_21.dt"
            result.uniformBlocks[0].glslUniforms[1].type = uniformTypeInt
            result.uniformBlocks[0].glslUniforms[1].arrayCount = 0
            result.uniformBlocks[0].glslUniforms[1].glslName = "_21.num_particles"
            result.views[0].storageBuffer.stage = shaderStageCompute
            result.views[0].storageBuffer.readonly = false
            result.views[0].storageBuffer.glslBindingN = 0
        of backendGles3:
            result.computeFunc.source = cast[cstring](addr(csUpdateSourceGlsl310es))
            result.computeFunc.entry = "main"
            result.uniformBlocks[0].stage = shaderStageCompute
            result.uniformBlocks[0].layout = uniformLayoutStd140
            result.uniformBlocks[0].size = 16
